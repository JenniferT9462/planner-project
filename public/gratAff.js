...
async function loadMindfulData() {
  try {
    ...
    DOM.gratitudeHistory.innerHTML =
      '<p class="text-center text-red-500 py-4">Error loading history.</p>';
...
function renderGratitudeHistory() {
  if (mindfulData.gratitudeHistory.length === 0) {
    DOM.gratitudeHistory.innerHTML =
      '<p class="text-center text-gray-500 py-4">No entries yet. Log something amazing!</p>';
    return;
  }

  const html = mindfulData.gratitudeHistory
    .map(
      (entry) => `
    ...
    .reverse()
    .join(""); // Reverse to show latest first
...
function displayRandomAffirmation() {
  const allAffirmations = [
    ...
    ...mindfulData.customAffirmations,
  ];
