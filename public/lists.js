...
function showPrompt(title, defaultValue = "", onSave) {
  ...
  if (!overlay) {
    ...
    overlay.className =
      "fixed inset-0 z-50 bg-black/70 **backdrop-blur-sm** flex items-center justify-center hidden opacity-0 transition-opacity duration-300";
    overlay.innerHTML = `
...
function hidePrompt() {
  ...
  if (overlay) {
    document
      .getElementById("customPromptContent")
...
async function loadLists() {
  ...
  try {
    ...
    console.warn(
      "Could not load lists from /api/lists (assuming first run or server not ready):",
      e
    );
...
function renderNewList(
  ...
  if (listType === "checklist") {
    ...
    createNote(
      ul,
      items[0] || "Click the edit button to start writing your note."
    );
  }
